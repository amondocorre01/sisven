...
// LOAD PRODUCT MODEL
$cobros_model = registry()->get('loader')->model('cobros');
...
if ($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'CREATE')
{
  try {
    ...
    //$product_id = $cobros_model->addProduct($request->post);
    var_dump('here');
    $product_id = $cobros_model->addCobros($request->post);

    exit();
    // get product info
    $product = $cobros_model->getProduct($product_id);
...
if ($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'UPDATE')
{
  try {
    ...
    // Edit product        
    $cobros_model->editProduct($p_id, $request->post);
...
if ($request->server['REQUEST_METHOD'] == 'POST' && isset($request->post['action_type']) && $request->post['action_type'] == 'DELETE')
{
  try {
    ...
    // Validate product id
    if (!validateInteger($request->post['id_cobro'])) {
    ...
    if ($request->post['delete_action'] == 'insert_to' && empty($request->post['id_cobro'])) {
      throw new Exception(trans('error_delete_product_name'));
    }

    // Fetch product by id
    $product = $cobros_model->getProduct($id_cobro);

    // Check product exist or not
    if (!isset($product['id_cobro'])) {
...
{
  $product = $cobros_model->getProduct($request->get['p_id']);
...
$table = "(SELECT products.*, p2s.*, suppliers.sup_mobile, suppliers.sup_name as supplier, boxes.box_name FROM products 
  ...
  $table = "(select * from cobros where estado='1') as cobros";
...
$columns = array(
  ...
  array( 
    ...
    'dt' => 'select' ,
    'formatter' => function($d, $row) {
        return '<input type="checkbox" name="selected[]" value="' . $row['id_cobro'] . '">';
    }
  ),
  array( 'db' => 'id_cobro', 'dt' => 'id_cobro' ),
  array( 
    'db' => 'p_image',   
    'dt' => 'p_image' ,
    'formatter' => function($d, $row) {
...
$columns = array(
    ...
    array( 'db' => 'nombre',   'dt' => 'nombre_cobro' ),
    array( 'db' => 'monto',   'dt' => 'monto_cobro' ,
    'formatter' => function($d, $row) {
        return currency_format($row['monto']);
      }
    ),
    array( 'db' => 'fecha',   'dt' => 'fecha_cobro' ),
    array( 'db' => 'notas_venta',   'dt' => 'notas_venta_cobro' ),
    array( 'db' => 'proximo_pago',   'dt' => 'proximo_pago_cobro' ),
    array( 
        'db' => 'id_cobro',   
        'dt' => 'delete_btn' ,
        'formatter' => function($d, $row) {
            //var_dump('row',$row);
             $iden = $row['id_cobro'];
          if ((DEMO && $row['id_cobro'] == 1)) {          
            return '<button class="btn btn-sm btn-block btn-default" type="button" disabled><i class="fa fa-trash"></i></button>';
          }
          return '<button class="btn btn-sm btn-block btn-danger" iden="'.$iden.'" onclick="confirmDelete(this)" type="button" title="'.trans('button_delete').'"><i class="fa fa-trash"></i></button>';
        }
      ),
