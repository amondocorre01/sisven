...
| -----------------------------------------------------
*/
class ModelCobros extends Model 
{
	public function addCobros($data){
		var_dump('datos',$data);
		$store_id = store_id();
		$obj = json_decode(json_encode($data, false));
		$this->db->query("insert into COBROS(nombre,monto,fecha,notas_venta,proximo_pago,estado,store_id) values('$obj->nombre','$obj->monto','$obj->fecha','$obj->nota_venta','$obj->proximo_pago','1','$store_id')");
