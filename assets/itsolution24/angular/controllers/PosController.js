...
function (
    ...
    $scope._percentage = function (amount, per)
    {
        if(false === $scope._isNumeric(amount) || false === $scope._isNumeric(per)) {
            ...
            window.toastr.error("El monto de descuento no es numerico!", "Aviso!");
            return 0;
        }
    ...
    $scope.showCustomerList = function (isClick) {
        ...
        then(function(response) {
            ...
            window.toastr.error(response.data.errorMsg, "Aviso!");
        });
    };
    ...
    $scope.addCustomer = function (customer) {
        if ($scope._isInt(customer)) {
            ...
            window.toastr.error("Oops!, Cliente invalido", "Aviso!");
        }
    };
    ...
    $scope._calcTotalPayable = function ($childScope) {
        ...
        if ($scope.payable != 0 && ($scope.discountAmount >= $scope.payable)) {
            ...
            window.toastr.error("El monto descuento debe ser menor que la cantidad a pagar.", "Aviso!");
        }
        if ($scope.payable != 0 && ($scope.shippingAmount >= $scope.payable)) {
            ...
            window.toastr.error("El importe del envío debe ser menor que la cantidad a pagar", "Aviso!");
        }
    ...
    $scope.addItemToInvoice = function (id, qty, index) {
        ...
        then(function(response) {
                    ...
                    window._.map($scope.itemArray, function (item) {
                        if (item.id == response.data.p_id) {
                            ...
                            if ((qty > response.data.quantity_in_stock || $scope.itemQuantity >= response.data.quantity_in_stock) && response.data.p_type != 'service') {
                                ...
                                window.toastr.error("Este producto está agotado!", "Aviso!");
                                return false;
                            }
                            if (window.store.sound_effect == 1) {
            ...
            window.onbeforeunload = function() {
              return "Data will be lost if you leave the page, Está seguro?";
            };
                ...
                window._.map($scope.itemArray, function (item) {
                    if (item.id == id) {
                        if (item.quantity > 1) {
                            ...
                            window.toastr.error("Cantidad no puede ser menor que 1", "Aviso!");
                        }
                    }
                });
            }
    ...
    $scope.removeItemFromInvoice = function (index, id) {
        ...
        if ($scope.isEditMode) {
            if ($scope.itemArray.length <= 1) {
                ...
                window.toastr.error("El último elemento no se puede eliminar!", "Aviso!");
                return false;
            }
        }
    ...
    $scope.loadHoldingOrder = function() {
        if (window.getParameterByName("holding_id")) {
            ...
            then(function(response) {
                ...
                window.toastr.warning(response.data.errorMsg, "Aviso!");
    ...
    $scope.clickOnPrintOrder = function () {
        if ($scope.itemArray.length <= 0) {
            ...
            window.toastr.error("Por favor, seleccione al menos un producto.", "Aviso!");
            return false;
        }
    ...
    $scope.payNow = function() {
        ...
        if (!$scope.customerName) {
            ...
            window.toastr.error("Por favor, seleccione un cliente", "Aviso!");
            return false;
        } 
                    ...
                    if (response.data.p_id) {
                        if ((itemquantity > response.data.quantity_in_stock || $scope.itemQuantity >= response.data.quantity_in_stock) &&  response.data.p_type != 'service') {
                            if ($scope.error == false) {
                                ...
                                window.toastr.error("Este producto esta agotado!", "Aviso!");
                            }
                        } else {
                            $scope.error = true;
                            $scope.triggerKeyup = true;
    ...
    $.contextMenu.types.label = function(item, opt, root) {
        ...
        .on("click", "button", function() {
            ...
            if (!itemQuantity || parseFloat(itemQuantity) <= 0) {
                ...
                window.toastr.error("Cantidad debe ser mayor que 0!", "Aviso!");
                return false;
            }
